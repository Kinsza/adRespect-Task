...
if ( ! function_exists( 'mytheme_comment_count' ) ) :
	...
	function mytheme_comment_count() {
		if ( ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
			...
			comments_popup_link( sprintf( __( 'Leave a comment<span class="sr-only"> on %s</span>', 'mytheme' ), get_the_title() ) );
		}
	}
endif;
	...
	function mytheme_entry_meta() {
		...
		edit_post_link(
			sprintf(
				wp_kses(
				/* translators: %s: Name of current post. Only visible to screen readers. */
					__( 'Edit <span class="sr-only">%s</span>', 'mytheme' ),
					array(
						'span' => array(
							'class' => array(),
						),
...
if ( ! function_exists( 'mytheme_comment_avatar' ) ) :
	/**
	 ...
	 */
	function mytheme_get_user_avatar_markup( $id_or_email = null ) {

		if ( ! isset( $id_or_email ) ) {
			$id_or_email = get_current_user_id();
		}

		return sprintf( '<div class="vcard">%s</div>', get_avatar( $id_or_email, mytheme_get_avatar_size() ) );
	}
endif;

if ( ! function_exists( 'mytheme_discussion_avatars_list' ) ) :
	/**
	 * Displays a list of avatars involved in a discussion for a given post.
	 *
	 * @param array $comment_authors Comment authors to list as avatars.
	 */
	function mytheme_discussion_avatars_list( $comment_authors ) {
		...
		foreach ( $comment_authors as $id_or_email ) {
			printf(
				"<li>%s</li>\n",
				mytheme_get_user_avatar_markup( $id_or_email ) // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			);
		}
		echo '</ol>', "\n";
	}
...
if ( ! function_exists( 'mytheme_the_posts_navigation' ) ) :
	...
	function mytheme_the_posts_navigation() {
		the_posts_pagination(
			array(
				'mid_size'  => 2,
				'prev_text' => __( 'Newer posts', 'mytheme' ),
				'next_text' => __( 'Older posts', 'mytheme' ),
			)
		);
	}
endif;

if ( ! function_exists( 'mytheme_content_class' ) ) :
	/**
	 ...
	 * MYTHEME_TYPOGRAPHY_CLASSES constant.)
	 *
	 ...
	 */
	function mytheme_content_class( $class = '' ) {
		$all_classes = array( $class, MYTHEME_TYPOGRAPHY_CLASSES );
